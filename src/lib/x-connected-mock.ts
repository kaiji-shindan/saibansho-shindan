// ============================================================
// Mock data for the "X account connected" state on the premium page.
// Renders the user's own attackers / hostile mentions / DMs once they
// have linked their X account via OAuth (Phase 3 future). Until OAuth
// is wired, the premium page uses this deterministic mock so the
// connection flow is fully demoable.
// ============================================================
...
export interface ConnectedAnalysis {
  totals: { attacks: number; attackers: number; high: number; medium: number; low: number };
  attacks: AttackItem[];
...
export function generateConnectedAnalysis(): ConnectedAnalysis {
  const capturedAt = NOW.toISOString();
  const attacks: AttackItem[] = RAW.map((r) => {
  ...
  const attackers: AttackerSummary[] = Array.from(map.values()).map((s) => {
    ...
    const w = { high: 100, medium: 30, low: 5 } as const;
    return (w[b.worstSeverity] ?? 0) * b.attackCount - (w[a.worstSeverity] ?? 0) * a.attackCount;
  });

  const sortedAttacks = [...attacks].sort((a, b) => {
    const sw = { high: 3, medium: 2, low: 1, none: 0 } as const;
